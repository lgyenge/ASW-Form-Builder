...
@Component({
  selector: 'au-textfield',
  templateUrl: './au-textfield.html',
})
export class AuTextField {
  ...
  @Output() textFieldUpdateEvent = new EventEmitter<{
  ...
  deleteTextFieldDialog(control: TextFieldControl, controlIndex: number): void {
    const dialogRef = this.dialog.open(AswConfirmDialog, {
      ...
      data: {
        ...
        message: this.constants.messages.waringMessage,
      },
    });
  ...
  editTextFieldDialog(control: TextFieldControl, controlIndex: number): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result !== undefined) {
        console.log(result);
        result.note = control.note;
        result.media = control.media;
